...
# tf.get_logger().setLevel('ERROR')   上一行有警告不想看的话，加一行这个
...
class ZINB(NB):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            result = tf.where(tf.less(y_true, 1e-6), zero_case, nb_case)
...
def pairwise_loss(anchor_output, positive_output, negative_output, margin=1):
